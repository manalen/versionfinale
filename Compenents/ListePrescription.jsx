...
class ListePrescritpion extends Component {
    constructor(props) {
        super(props)
        this.state = {
            Patients: [],
    ...
    refreshCourses() {
        PatientService.othmane2(this.state.id)
            .then(
                response => {
                    this.setState({ Patients:  response.data })
                }
            )
            PatientService.othmane3(this.state.id)
            .then(
                response => {
    ...
    mni(){
        this.props.history.push(`/med/${this.state.idmedecin}/ajouterprescription/${this.state.id}`)
        console.log('voila')
        ...
        return (
            <div>
                ...
                <div className="container">
                            ...
                            <tbody>
                                {
                                    this.state.Patients.map(
                                        patient =>
                                        <tr key={patient.idpres}>
                                             <td>{patient.medicament_prescrit}</td>
                    
                                            <td>{patient.date_debut_traitement}</td>
                                            <td>{patient.date_fin_traitement}</td>
                                            <td>{patient.motif_pres}</td>
                    ...
                    <div>
                    <button  onClick={this.mni} className="btn btn-success" style={{backgroundColor:"#435fbc", borderColor:"#435fbc",position:"absolute",top:"200px",height: "50px",left:"980px"}}>Ajouter une prescription </button>
...
export default ListePrescritpion